...
if (!function_exists('core21_legacy_board_icon'))
{
	function core21_legacy_board_icon($board)
	{
		...
		$cache_buster = in_array($image, array('on.gif', 'on2.gif', 'off.gif', 'redirect.gif'), true) ? '?core21=board-icons-20260608' : '';
		echo '<a href="', $href, '"><img src="', $settings['images_url'], '/', $image, $cache_buster, '" alt="', $title, '" title="', $title, '"></a>';
	}
}
...
function template_main()
{
	...
	{
		if (empty($category['boards']) && empty($category['is_collapsed']))
			continue;

		echo '
	<div class="categoryframe tborder clearfix">
		<h3 class="catbg', !empty($category['new']) ? '2' : '', '">';

		if (!$context['user']['is_guest'] && !empty($category['show_unread']))
			echo '
			<a class="floatright" href="', $scripturl, '?action=unread;c=', $category['id'], '">', $txt['view_unread_category'], '</a>';

		if (!empty($category['can_collapse']))
			echo '
			<span id="category_', $category['id'], '_upshrink" class="', !empty($category['is_collapsed']) ? 'toggle_down' : 'toggle_up', ' floatright" data-collapsed="', (int) !empty($category['is_collapsed']), '" title="', !empty($category['is_collapsed']) ? $txt['show_category'] : $txt['hide_category'], '" style="display: none;"></span>';

		echo $category['link'], '
		</h3>';

		if (empty($category['is_collapsed']))
		{
			echo '
		<table cellspacing="1" class="bordercolor boardsframe">';
			...
			<tr>
				...
				<td class="windowbg2 info">
					<h4><a href="', $board['href'], '" id="b', $board['id'], '">', $board['name'], '</a>';

				if (!empty($board['can_approve_posts']) && (!empty($board['unapproved_posts']) || !empty($board['unapproved_topics'])))
					echo '
						<a href="', $scripturl, '?action=moderate;area=postmod;sa=', (!empty($board['unapproved_topics']) ? 'topics' : 'posts'), ';brd=', $board['id'], ';', $context['session_var'], '=', $context['session_id'], '" title="', sprintf($txt['unapproved_posts'], $board['unapproved_topics'], $board['unapproved_posts']), '" class="moderation_link">(!)</a>';
				...
				<td', !empty($board['children']) ? ' rowspan="2"' : '', ' class="windowbg stats smalltext">
					', comma_format($board['posts']), ' ', $is_redirect ? $txt['redirects'] : $txt['posts'], ' <br>
					', $is_redirect ? '' : comma_format($board['topics']) . ' ' . $txt['board_topics'], '
				</td>
				<td', !empty($board['children']) ? ' rowspan="2"' : '', ' class="windowbg2 smalltext lastpost">';

				if (!empty($board['last_post']['id']))
					echo !empty($board['last_post']['last_post_message']) ? $board['last_post']['last_post_message'] : (!empty($board['last_post']['link']) ? $board['last_post']['link'] : '');
				...
				if (!empty($board['children']))
				{
					...
					{
						$is_child_redirect = !empty($child['is_redirect']) || (!empty($child['type']) && $child['type'] === 'redirect');
						if (!$is_child_redirect)
							$link = '<a href="' . $child['href'] . '" title="' . (!empty($child['new']) ? $txt['new_posts'] : $txt['old_posts']) . ' (' . $txt['board_topics'] . ': ' . comma_format($child['topics']) . ', ' . $txt['posts'] . ': ' . comma_format($child['posts']) . ')">' . $child['name'] . '</a>';
						else
						...
						if (!empty($child['can_approve_posts']) && (!empty($child['unapproved_posts']) || !empty($child['unapproved_topics'])))
							$link .= ' <a href="' . $scripturl . '?action=moderate;area=postmod;sa=' . (!empty($child['unapproved_topics']) ? 'topics' : 'posts') . ';brd=' . $child['id'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '" title="' . sprintf($txt['unapproved_posts'], $child['unapproved_topics'], $child['unapproved_posts']) . '" class="moderation_link">(!)</a>';

						$children[] = !empty($child['new']) ? '<strong>' . $link . '</strong>' : $link;
					}
	...
	<div id="modbuttons_bottom" class="modbuttons clearfix">
		<div id="posticons" class="clearfix marginbottom">
			<div class="smalltext floatleft">
				<img src="', $settings['images_url'], '/new_some.gif" alt="" style="vertical-align: middle;"> ', $txt['new_posts'], '
				<img src="', $settings['images_url'], '/new_none.gif" alt="" style="vertical-align: middle; margin-left: 4ex;"> ', $txt['old_posts'], '
			</div>';

		if (!empty($context['mark_read_button']))
			template_button_strip($context['mark_read_button'], 'top');
