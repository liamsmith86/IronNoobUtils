...
function template_profile_above()
{
	...
	// The core21 theme uses a simplified profile layout, so expose the most
	// important edit destinations explicitly for profile owners.
	if (!empty($context['user']['is_owner']))
	{
		global $scripturl, $txt;

		echo '
			<div class="buttonlist profile_edit_links">
				<a class="button" href="', $scripturl, '?action=profile;area=account;u=', $context['id_member'], '">', isset($txt['account']) ? $txt['account'] : 'Account Settings', '</a>
				<a class="button" href="', $scripturl, '?action=profile;area=forumprofile;u=', $context['id_member'], '">', isset($txt['forumprofile']) ? $txt['forumprofile'] : 'Edit Profile', '</a>
			</div>';
	}
}
